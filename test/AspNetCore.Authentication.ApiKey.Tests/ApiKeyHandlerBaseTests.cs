...
namespace AspNetCore.Authentication.ApiKey.Tests
{
	public class ApiKeyHandlerBaseTests 
	{
...
#if !(NET461 || NETSTANDARD2_0 || NETCOREAPP2_1)
				services.Configure<AuthorizationOptions>(options => options.FallbackPolicy = new AuthorizationPolicyBuilder([.. schemes]).RequireAuthenticatedUser().Build());
#endif
			});
		...
		private class FakeApiKeyProviderLocal_1 : IApiKeyProvider
		{
			...
			{
				return Task.FromResult((IApiKey?)new FakeApiKey(key, "Test", [new("Provider", "1")]));
			}
		}
		...
		private class FakeApiKeyProviderLocal_2 : IApiKeyProvider
		{
			public Task<IApiKey?> ProvideAsync(string key)
			{
				return Task.FromResult((IApiKey?)new FakeApiKey(key, "Test", [new("Provider", "2")]));
			}
		}
	}
}
