...
{
	public class ApiKeyInRouteValuesHandlerTests : IDisposable
	{
		...
		{
			_server = TestServerBuilder.BuildInRouteValuesServer();
			_client = _server.CreateClient();

			_serverWithProvider = TestServerBuilder.BuildInRouteValuesServerWithProvider();
		...
		public async Task Verify_Handler()
		{
			...
			Assert.Equal(typeof(ApiKeyInRouteValuesHandler), scheme.HandlerType);
		...
		public async Task Verify_challenge_www_authenticate_header()
		{
			...
			Assert.Equal($"realm=\"{TestServerBuilder.Realm}\", charset=\"UTF-8\", in=\"route_values\", key_name=\"{FakeApiKeys.KeyName}\"", wwwAuthenticateHeaderToMatch.Parameter);
		}
		...
		{
			var uri = $"{TestServerBuilder.BaseUrl}route/{FakeApiKeys.FakeKey}";
		...
		{
			var uri = $"{TestServerBuilder.BaseUrl}route/{FakeApiKeys.FakeInvalidKey}";
			using var request = new HttpRequestMessage(HttpMethod.Get, uri);
			using var response = await _client.SendAsync(request);
			Assert.False(response.IsSuccessStatusCode);
			Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
		...
		// ~ApiKeyInRouteValuesHandlerTests()
		// {
